...
export default class Thread {
    ...
    private currentStepData(): any | null {
    ...
    private playingPlayerNames(): string[] {
    ...
    private presentatorName(): string | undefined {
    ...
    private refereeName(): string | undefined {
    ...
    private isDynamicThemeStep(step: any): boolean {
     ...
     * préparés à l'avance.
     */
    private allDynamicStepsFilled(): boolean {
        const steps = this.room?.emission?.steps;
     ...
     * ne joue plus, donc ne doit ni bloquer le lancement (gate ci-dessous) ni
     * être compté dans la grille — c'est son thème qui doit au contraire
     * nourrir le pool neutre (cf. startGrid).
     */
    private activeEmissionPlayerNames(): string[] {
    ...
    private finishStep(ranking: { name: string; score: number }[]): void {
        ...
        if (this.room.withPresentator) {
            ...
            io.to(String(this.room.id)).emit("emission:bonusMalus", { ranking });
    ...
    private finalizeStep(ranking: { name: string; score: number }[], adjustments: Record<string, number>): void {
        ...
        if (nextStep) {
            io.to(roomId).emit("emission:stepFinished", {
                ranking,
            ...
            io.to(roomId).emit("emission:finished", { ranking: finalRanking });
     ...
     */
    private startTeamFormation(step: any): void {
    ...
    private async startList(step: any): Promise<void> {
        ...
        await this.list.start(this.playingPlayerNames());
    }
    ...
    private async startGrid(step: any): Promise<void> {
        ...
        if (this.isDynamicThemeStep(step)) {
            const baseConfig = {
            ...
            playerNames = this.activeEmissionPlayerNames();
            ...
            playerNames = this.playingPlayerNames();
    ...
    private async startPickBan(step: any): Promise<void> {
        ...
        await this.pickBan.start(this.playingPlayerNames());
    }
    ...
    private async startDuel(step: any): Promise<void> {
        ...
        const participants = this.resolveDuelParticipants();
        if (!participants) {
            io.to(String(this.room.id)).emit("duel:error", {
    ...
    private async startPoints(step: any): Promise<void> {
        ...
        await this.points.start(this.playingPlayerNames());
    }
    ...
    private async startBR(step: any): Promise<void> {
        ...
        await this.br.start(this.playingPlayerNames());
    }
     ...
     * 2 participants.
     */
    private resolveDuelParticipants(): [string, string] | null {
        const teams = this.room.getTeams();
        if (teams.length > 0) {
    ...
    public sendStateTo(username: string): void {
        ...
        // attente ne revoit plus jamais cet écran (émis une seule fois).
        if (this.pendingStepRanking) {
            io.to(String(this.room.id)).emit("emission:bonusMalus", { ranking: this.pendingStepRanking });
        }
    }
    ...
    public setShowView(requester: string, view: ShowView): boolean {
        ...
        this.showView = view;
        io.to(String(this.room.id)).emit(SHOW_EVENTS.state, { view });
        return true;
    }
     ...
     * même quand l'étape courante n'a pas `resetPoint` (cf. `start()`) — un
     * filet de rattrapage manuel pour un cas non prévu au montage de l'émission.
     */
    ...
    private broadcastRoomInfo(): void {
        io.to(String(this.room.id)).emit("infoRoom", this.room.getInfo());
    }
