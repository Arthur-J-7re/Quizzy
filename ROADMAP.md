...
- **Blueprints d'émission** (12 Coups de Midi, Questions pour un Champion) — `Interface/Step.ts` semble assez générique pour porter des templates prédéfinis de `Step[]`, à investiguer plus précisément le moment venu.
  - **Résolu le 2026-09-11** : `shared-types/emission.ts` porte les 2 templates (`EMISSION_BLUEPRINTS`) et `recalcStepCounts` (déplacé depuis `Emission.tsx` pour être partagé/testable). Un sélecteur sur l'écran de création préremplit title/options/steps ; le résultat reste une émission normale, librement éditable, réservé à la création (jamais appliqué par-dessus une émission existante). Assertions structurelles en vitest (`server/function/emissionBlueprints.test.ts`) : bornes joueurs/étapes du formulaire, modes réellement sélectionnables dans l'éditeur, point fixe de `recalcStepCounts`. N'utilise que LIST/DUEL/TIMER — POINTS et BR existent côté moteur (`_Thread.ts`) mais n'ont pas encore d'entrée dans le sélecteur de mode d'étape (`StepForm.tsx`, `AVAILABLE_MODES`), donc exclus des templates tant que ce n'est pas fait.
