...
- **Constat au 2026-08-18 : cet addendum n'a pas été traité, la duplication prédite s'est bien produite.** Les phases 2 à 6 ont été implémentées (non commitées) directement dans les 4 managers plutôt que via un service commun. Preuve concrète : `getPublicationStatus`, `getCreatorOfX`, `getXByIds` et `getAvailableX` existent maintenant en 3 copies quasi identiques (`quizzManager.ts`, `themeManager.ts`, `emissionManager.ts` — `questionManager.ts` a une variante propre à la modération avec `getPendingBacklog`/`approveQuestion`/`rejectQuestion`). Aucun fichier `entityManager`/CRUD générique n'existe dans `server/function/` ou `server/utils/`.
  - **Pourquoi ce n'est toujours pas fait** : le graphe de dépendances du roadmap plaçait la phase 1 "en parallèle, aucune dépendance croisée" — rien ne bloquait donc les phases 2-6 d'avancer sans elle, et elles ont été développées avant elle plutôt qu'après.
  - **Coût du retard** : factoriser maintenant veut dire refactorer 4 managers déjà fonctionnels et testés (cf. `_Thread.test.ts` etc.) plutôt que d'écrire le service une fois puis de brancher les 4 dessus. À faire avant d'ajouter une 5ᵉ entité au pattern (ex. futurs blueprints d'émission, backlog non planifié).
